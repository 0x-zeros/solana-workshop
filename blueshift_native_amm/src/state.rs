...
pub struct Config {
    state: u8,
    seed: [u8; 8],
    authority: Pubkey, //对 AMM 拥有管理控制权的公钥（例如，用于暂停或升级池）。可以通过传递 [0u8; 32] 将其设置为不可变。
    mint_x: Pubkey,
    mint_y: Pubkey,
    fee: [u8; 2], //以基点（1 基点 = 0.01%）表示的交换费用，在每次交易中收取并分配给流动性提供者。
    config_bump: [u8; 1], //用于 PDA 派生的 bump 种子，确保配置账户地址有效且唯一。保存此值以提高 PDA 派生效率。
}
